...
export default function ReportReadiness({ filters, data }) {
  ...
  const downloadSummary = () => {
    const summary = [
      ...
      `Estimated businesses: ${data.competitionCount ?? 'Not available'}`,
      `Local business environment: ${data.localEnvironmentScore ?? 'Not available'}/100`,
      '',
      'These figures are advisory market signals. Verify important assumptions before investing.',
    ].join('\n')
