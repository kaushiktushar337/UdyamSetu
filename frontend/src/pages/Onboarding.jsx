...
export default function Onboarding() {
  ...
  const resolveLocation = (result) => {
    if (!result?.estimated_address) return
    setForm((current) => ({ ...current, location: result.estimated_address, locationId: result.location_id || current.locationId }))
  }
        ...
        <div className="soft-card mt-8 p-5 sm:p-8">
          {step === 0 && (
            <div>
              ...
              <div className="mt-6"><CurrentLocationButton onResolved={resolveLocation} /></div>
              {form.location && <div className="mt-3 rounded-xl bg-[#f2f7ee] p-3 text-xs text-neutral-600"><span className="font-semibold text-neutral-900">Selected area:</span> {form.location}</div>}
